...
passUnsealTokenToRootGeneration() {
  ...
  ENC_ROOT_TOKEN=$(vault operator generate-root -otp="$2" -nonce="$3" -format=json "$UNSEAL_TOKEN" | jq -r .encoded_root_token | tr -d '\r')
}

rootLogin() {
  ROOT_TOKEN=$(openssl enc -a -d -aes-128-cbc -pass env:HCV_SEALFILE_PASS -in "$UNSEAL_FILE" |
    jq -r .root_token | tr -d '\r')

  vault login token="$ROOT_TOKEN" >/dev/null
}

generateRootToken() {
  OTP=$(vault operator generate-root -generate-otp -format=json | jq -r .otp | tr -d '\r')
  NONCE=$(vault operator generate-root -init -otp="$OTP" -format=json | jq -r .nonce | tr -d '\r')
  ...
  ROOT_TOKEN=$(vault operator generate-root -nonce="$NONCE" -otp="$OTP" -decode="$ENC_ROOT_TOKEN" -format=json | jq -r .token | tr -d '\r')
}
